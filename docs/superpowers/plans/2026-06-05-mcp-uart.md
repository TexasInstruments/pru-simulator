...
**Goal:** Write `docs/mcp_server.md` (setup guide + UART examples), `source/uart_tx.asm` (single byte), `source/uart_print.asm` (null-terminated string), tests for both ASM files, and add the `pru-simulator` MCP server entry to Claude Code settings.
...
| Modify | `~/.claude/settings.json` — add `mcpServers` |
...
Add a `mcpServers` entry to `~/.claude/settings.json` (merge into the existing
...
## Task 4: Add MCP Server to Claude Code Settings

**Files:**
- Modify: `C:/Users/a0746725/.claude/settings.json`

- [ ] **Step 1: Read the current settings.json**

Confirm the file does not already contain a `mcpServers` key.

- [ ] **Step 2: Add `mcpServers` entry**

Open `C:/Users/a0746725/.claude/settings.json` and insert the `mcpServers`
block as a new top-level key. The final JSON must remain valid.
...
```bash
python -c "import json; json.load(open('C:/Users/a0746725/.claude/settings.json')); print('valid')"
```
...
Restart the Claude Code session. The `pru-simulator` MCP server is now active.
You can ask Claude to load and run `source/uart_tx.asm` directly in the chat.
