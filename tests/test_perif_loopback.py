...
def _setup(drift_ppm=0.0, latency_ns=0.0):
    ...
    set_rxcfg(pru1.registers, sample_size=7, sb_pol=1, clk_sel=1, div=0)
    set_ch_cfg0(pru1.registers, 0, rx_frame=1)          # EOF after 1 byte
...
def test_clean_roundtrip():
    ...
    pru1.channels[0].arm_rx(True)
    pru1.channels[0]._rx_next_edge_ns = 0.5 * T          # sample mid-bit (phase align)
    pru1.advance(9 * T)                                  # exactly the 9-bit frame window

    assert pru1.channels[0].rx_fifo == [V]               # round-trips the byte
    assert pru1.channels[0].rx_eof is True
...
def test_loopback_disabled_no_rx():
    ...
    pru0.advance(20 * T)
    pru1.channels[0].arm_rx(True)
    pru1.channels[0]._rx_next_edge_ns = 0.5 * T
    pru1.advance(20 * T)
    assert pru1.channels[0].rx_fifo == []                # line idles low, no start bit
...
def test_large_drift_causes_slip():
    ...
    s, pru0, pru1 = _setup(drift_ppm=100.0)
