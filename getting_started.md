...
In peripheral mode the GPO/GPI lanes are **not** drawn: the GP Mux has handed
those pads to the Peripheral Interface, so R30/R31 bits no longer describe
anything on the wire. The graph shows three lanes per active channel instead —
`perifN_out` (the serial data line), `perifN_out_en` (asserted while the channel
is actually driving the pad) and `perifN_clk` (its bit clock). A channel appears
as soon as any one of its three signals toggles, and then all three lanes are
drawn, so a steady `out_en` stays visible next to the data it qualifies.

The perif serializer sends raw MSB-first bits with no framing of its own, so use
the clock lane to find bit boundaries — there is one clock edge per bit, and the
data line is stable across each half-period. At the usual `TXCFG` divider
